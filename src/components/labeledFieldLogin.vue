<template>
    <v-flex v-bind:[xsValue]="true" :class="classes" class="pt-0 pb-0">
        <v-card flat text tile :class="{'mt-1': hideLabel}" class="c-labeled-field-login">
            <span v-if="label && !hideLabel" class="c-labeled-field-login__label fs-s nonselect" :class="[isError ? 'red500' : '' ]">{{label}}<span v-if="required && editMode" class="red500">*</span></span><br v-if="label && !hideLabel">
            <span class="fs-m" >
                    ...
                    <v-flex v-bind:[xsMain]="true">
                        ...
                        <div class="relative">
                            <input v-if="masked"
                                :type="typeInner" 
                                :value="text" 
                                class="c-labeled-field-login__input fs-s"
                                :class="[isError ? 'red500 red500-border' : '', inputIcon ? 'pr-6' : '', leftText ? 'text-right' : '']"
                            ...
                            <input v-else
                                ...
                                @keyup="checkCaps($event)"
                            ><!-- v-imask="mask" -->
                            <v-btn v-if="inputIcon && !advancedMode" text tile class="white elevation-0 c-labeled-field-login__btn"><i class="grey500-i icon-Create py-1"   @click="applyClick"></i></v-btn>
                            <v-btn v-if="inputIcon && advancedMode"  text tile class="white elevation-0 c-labeled-field-login__btn"><i class="grey500-i icon-Create py-1"   @click="applyClick"></i></v-btn>
                            <v-btn v-if="showButton && value != ''"  text tile class="white elevation-0 c-labeled-field-login__btn" @mousedown="typeInner='text'" @mouseup="typeInner='password'" @mouseout="typeInner='password'"><i class="grey500-i icon-eye icon-14 py-1" :class="{'red500-i': isError}"></i></v-btn>
                        </div>
                        </form>
...
.c-labeled-field-login
    line-height: 1.1
    background: transparent !important
    & &__label
        color: var(--white-primary700) !important
    & &__input
        height: 24px
        width: 100%
        border-bottom: 1px solid var(--white-primary500)
        color: var(--white-primary900) !important
        padding: 4px 8px
        font-weight: 600
    & &__btn
        position: absolute
