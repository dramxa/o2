<template>
    <d-dialog-basic :dialogId="id" maxWidth="500px">
        <v-card class="d-save-as-template pa-6 white-light gd-dlg-saveTmpl">
            <v-layout>
                <v-flex>
                    <v-card-text class="fs-l pa-0 grey700"> 
                        <v-layout>
                            <v-flex>
                                Save <span class="blue700 fw-b">{{sourceName}}</span> as template
                            </v-flex>
                            <v-flex class="d-save-as-template__close-btn-wrapper">
                                <v-btn icon tile text width="36" height="36" class="d-save-as-template__close-btn br-4 gd-dlg-close" @click="cancelClick"><i class="icon-Close icon-24"></i></v-btn>
                            </v-flex>
                        </v-layout>
                        <v-layout class="mt-4 gd-dlg-saveTmpl-name">
                            <c-labeled-field class="px-0" xs="11" label="NAME" v-model="name" editMode="true"/>
                        </v-layout>                        
                        <v-layout class="mt-4 gd-dlg-saveTmpl-comment">
                            <c-labeled-textArea class="px-0" xs="11" label="COMMENT" v-model="comment" editMode="true"/>
                        </v-layout>
                        <v-layout class="mt-4 gd-dlg-saveTmpl-locked">
                    ...
                    <v-card-actions class="pa-0 mt-6">
                        ...
                        <v-btn class="blue700-b normal-text fs-ml elevation-0 white--text" small width="80" height="32" @click="clickYes"> Save</v-btn>
                    </v-card-actions>
                </v-flex>
            </v-layout>
        </v-card>
...
export function register(){
    Vue.prototype.$windows.registerDialog({ dialogCode: 'saveAsTemplate', component})
}
