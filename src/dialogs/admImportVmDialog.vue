<template>
    <d-dialog-basic :dialogId="id" maxWidth="500px">
        <v-card class="d-adm-import-vm pa-6 white-light gd-dlg-import">
            <v-layout>
                <v-flex>
                    <v-card-text class="fs-l pa-0 grey700"> 
                        ...
                        <v-flex class="d-adm-import-vm__close-btn-wrapper">
                            <v-btn icon tile text width="36" height="36" class="d-adm-import-vm__close-btn br-4" @click="cancelClick"><i class="grey500-i icon-Close icon-24"></i></v-btn>
                        </v-flex>
                        </v-layout>
                        <v-layout class="mt-4">
                            <c-labeled-field class="px-0 gd-dlg-import-name" xs="12" label="NAME" v-model="name"  editMode="true" required="true"/>
                        </v-layout>
...
const component = Vue.extend({
    ...
    methods:{
        clickYes(){
            if(this.name == null || this.name == '' || isNullOrUndefined(this.resourceProfile)){
...
export function register(){
    Vue.prototype.$windows.registerDialog({ dialogCode: 'admImportVmDialog', component})
}
