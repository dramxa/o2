...
<template>
  <div>
    ...
    <v-data-table
      ...
      hide-default-header
      hide-default-footer
      class="grp-table custom-table gd-table"
      fixed-header
    > 
      <template v-slot:header="data">
        <thead>
          ...
          <th v-if="header.permission || header.value == 'data-table-select'" :class="[header.value == 'data-table-select' ? 'gd-tb-col gd-tbcol-select' : header.guideCls, 'column sortable', descending ? 'desc' : 'asc', header.value === sortBy ? 'active' : '', (header.value != 'data-table-select' && header.show != 'all' && !$vuetify.breakpoint[header.show]) || (glb_showProperty && header.value != 'data-table-select' && header.showWithProps != 'all' && !$vuetify.breakpoint[header.showWithProps]) ? 'd-none' : '']" :key="header.value" class="table-header-text pa-2 fw-n" :width="header.width"><!-- :style="{width: 'calc(' + header.width + ' + ' + header.rightSpacer + ') !important'}" -->
                    ...
                    <template v-slot:activator="{ }">
                      <v-tooltip bottom  content-class="custom-tooltip fs-s">
                        <template v-slot:activator="{ on }">
                          <v-btn text v-on="on" @click.stop="menuPeriod=true" :class="[sortBy == header.value ? 'fw-b grey800 fs-s': 'grey600 fs-s']" class="normal-text mb-0-5 mx-1 px-0 border-none br-4 gd-tb-price-btn" min-width="unset" height="24px">(<span class="grp-table__price-period grey800-border border-bottom-dashed">{{glb_pricePeriod}}</span>)</v-btn>
                        </template>
            ...
            <div v-else :class="{'mt-2': !useFilter}">
                <c-table-checkbox 
                  logging="true" 
                  :state="selected.length==grp_itemsFiltered.length && grp_itemsFiltered.length > 0"  
                  @click="updateSelectedOnClick(data, $event)" 
                  class="ml-2 d-flex align-center justify-space-around mt-0"  
                  :cbid="'grp-table-header-cb'" 
                  :indeterminate="selected.length>0 && selected.length < grp_itemsFiltered.length"
                 />             
                <div v-if="useFilter" class="grp-table__item-counter mt-3 grey900 text-left">{{selected.length}}/{{grp_itemsFiltered.length}}</div>
            </div>
          </th>
          </template>
          </tr>
          <tr v-if="useFilter&&chipsSorted.length>0">
            <td :colspan="headers.length + 1" class="border-bottom grey200-border">
              <v-chip v-for="(chip, index) in chipsSorted" :key="'filter-chips-' + index" @click:close="chip.clear()" close close-icon="icon-Close blue700-i icomoon-important close-chips-button br-4" class="grp-table__filter-chip br-4 blue800 blue000-b fs-s fw-b px-2 ml-4">{{chip.type}}: {{chip.text}}</v-chip>
            </td>
          </tr>
        </thead>
      </template>
        ...
        <tr :class="rowSelectedClass(data.item)" class="row-with-hidden-menu">
          <template v-for="(header, index) in data.headers">
            <td :key="index" v-if="index==0" class="gd-tbcol-row-select-cell" :class="[header.align == 'right' ? 'monospace' : '', ' pointer text-' + header.align]" @click="data.select(!data.isSelected), updateSelectedOnClick()">
              <div>
                <c-table-checkbox class="gd-tbcol-row-select-cb" :cbid="'grp-table-row-' + data.item.id"  :state="data.isSelected" @click="data.select(!data.isSelected), updateSelectedOnClick()"/>
             </div>
            </td>
          <td v-if="header.permission && index!=0" :key="index" :class="[header.isMono ? 'monospace' : '', header.value == 'name' ? ' pointer' : '', ' text-' + header.align, (header.value != 'data-table-select' && header.show != 'all' && !$vuetify.breakpoint[header.show]) || (glb_showProperty && header.value != 'data-table-select' && header.showWithProps != 'all' && !$vuetify.breakpoint[header.showWithProps]) ? 'd-none' : '']" :style="{paddingRight: 'calc(16px + ' + header.rightSpacer + ') !important'}">
              ...
              <v-layout class="d-flex align-strech fill-height">
                <div @click="showDetails(data.item.id, false)" @click.middle="showDetails(data.item.id, true)" class="d-flex align-senter fill-height">
                  <div class="align-self-center">
                    <label class="fw-b poiter blue800 name-overflow">{{data.item.name}}</label>
                    <div class="grp-table__description-overflow fs-s grey700">{{data.item.description}}</div>
                  </div>
                </div>
                <v-spacer @click="showDetails(data.item.id, false)" @click.middle="showDetails(data.item.id, true)"></v-spacer>
                <v-menu right offset-x width="240px" v-model="data.item.menu" :close-on-content-click="false" v-if="permission('ft_grpDelete')">
                  ...
                  <div class="grp-table__row-menu border blue050-border br-4 white-theme-table-menu gd-tb-row-item-menu">
                  <v-layout class="nowrap bg-gradient px-4 py-1 br-4-t">
                    <span class="grp-table__row-menu-name white--text">{{data.item.name}}</span>
                  </v-layout>
                  <v-list  class="control-combobox py-2 br-4-b">
                    ...
                    <v-list-item height="32px" class="fs-m gd-tb-row-item-menu-delete" @click="deleteClick(data.item)"><i class="icon-trash icon-16 mr-2"></i>Delete</v-list-item>
                  </v-list>
                  </div>
                </v-menu>
              </v-layout>
            ...
            <div v-if="header.value=='cpu'">
              <span class="grey900 fs-m">{{isNullOrUndefined(data.item.prep.cpuTotal)? '': data.item.prep.cpuTotal}}</span>
            </div>

            <!-- ram -->
            <div v-if="header.value=='ram'">
              <span class="grey900">{{isNullOrUndefined(data.item.prep.ramTotalFormatted) ? '': data.item.prep.ramTotalFormatted}} GB</span>
            </div>
...
import { objStatusCode,objStatus } from '@/consts/_otherConsts';
import grpToolBar                  from './grp-view-toolBar'
...
export default {
    ...
    data () {
      return {
        ...
        headers: [
          { text: 'GROUP NAME', guideCls: 'gd-tb-col gd-tbcol-name'   , show: 'all'    , showWithProps: 'all'    , align: 'left' , isMono:false, value: 'name'                     , filterType:'input'  , isLeft:false, permission: true, width: 'auto' },
          { text: 'PROJECT'   , guideCls: 'gd-tb-col gd-tbcol-project', show: 'all'    , showWithProps: 'all'    , align: 'left' , isMono:false, value: 'prep.projectName'         , filterType:'input'  , isLeft:false, permission: true, width: 'auto' },
          { text: 'VM#'       , guideCls: 'gd-tb-col gd-tbcol-vm'     , show: 'all'    , showWithProps: 'all'    , align: 'right', isMono:true , value: 'prep.vmCount'             , filterType:'from-to', isLeft:false, permission: this.permission('ft_vmRead'), width: '70px' },
          { text: 'STATUS'    , guideCls: 'gd-tb-col gd-tbcol-status' , show: 'mdAndUp', showWithProps: 'lgAndUp', align: 'left' , isMono:false, value: 'statusCode'               , filterType:'select' , isLeft:false, permission: true, width: '150px' },
          { text: 'CPU'       , guideCls: 'gd-tb-col gd-tbcol-cpu'    , show: 'all'    , showWithProps: 'mdAndUp', align: 'right', isMono:true , value: 'cpu'                      , filterType:'from-to', isLeft:false, permission: true, width: '70px'  },
          { text: 'RAM(GB)'   , guideCls: 'gd-tb-col gd-tbcol-ram'    , show: 'all'    , showWithProps: 'mdAndUp', align: 'right', isMono:true , value: 'ram'                      , filterType:'from-to', isLeft:false, permission: true, width: '110px' },
          { text: 'CREATED'   , guideCls: 'gd-tb-col gd-tbcol-created', show: 'smAndUp', showWithProps: 'lgAndUp', align: 'left' , isMono:true , value: 'createDate'               , filterType:'from-to', isLeft:true , permission: true, width: '150px', fromToType:'date'},
          { text: 'PRICE'     , guideCls: 'gd-tb-col gd-tbcol-price'  , show: 'all'    , showWithProps: 'all'    , align: 'right', isMono:true , value: 'prep.price'               , filterType:'from-to', isLeft:true , permission: this.permission('ft_prcRead'), width: '140px'  },
          { text: ' '                                                 , show: 'all'    , showWithProps: 'xl'     , align:'left' , value: 'spacer', isMono:true , permission: true, width: '3%'},
        ]
      }
    },

      computed:{
        ...mapGetters(['grp_itemsFiltered', 'grp_itemsSelected', 'grp_filter', 'glb_showProperty', 'grp_showProperty', 'grp_reactiveState', 'glb_pricePeriod']),

        filter:{
          get: function(){        
            return this.grp_filter;
          },

          set: function(data){
            this.grp_setFilter(data);
          }
        }
      },
      ...
      watch:{
        selected: function (data) {
          let compare = this.grp_itemsSelected.filter( function(e){
            return this.indexOf(e) < 0
          },  data.map(i=>i.id));
          if(this.userSelect){
            this.grp_setSelectedItems(data.map(item=>item.id));
            this.userSelect = false;
          }
        },

        grp_itemsSelected: function(data){
          this.selected = this.grp_itemsFiltered.filter(f=>data.includes(f.id))
        },

        grp_itemsFiltered:{
          handler: function(items){
            this.selected = this.grp_itemsFiltered.filter(f=>this.grp_itemsSelected.includes(f.id))
          },
          deep: true
        }
      },
      ...
      methods:{
        ...mapActions(['grp_getItems', 'grp_setSelectedItems', 'grp_setFilter', 'grp_deleteConfirm', 'grp_setEdited', 'grp_setShowProperty', 'glb_setPricePeriod']),
        ...
        editProperty(id){
          this.grp_setEdited(id);
        },
        showProperty(id){
          this.grp_setShowProperty(id);
        },
        ...
        rowSelectedClass(item){
          ...
          var ir = this.grp_reactiveState.find(f=>f.id==item.id);
          ...
          if(this.selected.includes(item))
           return 'blue100-b';
          if(!isNullOrUndefined(this.grp_showProperty) && this.grp_showProperty.id==item.id)
           return 'blue000-b';
          return '';
        },

        deleteClick(item){
          this.grp_deleteConfirm(item);
        },
        ...
        gotoVms(item, newWindow){
          this.$nav.navigateTo('vms', { use: true, projectName: item.prep.projectName, groupName: item.name }, newWindow)
        },

        gotoErrorVms(item, newWindow){
          this.$nav.navigateTo('vms', { use: true, projectName: item.prep.projectName, groupName: item.name, statuses: ['error'] }, newWindow)
        },
        
        gotoBusyVms(item, newWindow){
          this.$nav.navigateTo('vms', { use: true, projectName: item.prep.projectName, groupName: item.name, statuses: ['busy'] }, newWindow)
        },

        gotoReadyVms(item, newWindow){
          this.$nav.navigateTo('vms', { use: true, projectName: item.prep.projectName, groupName: item.name, statuses: ['ready'] }, newWindow)
        }
      },
