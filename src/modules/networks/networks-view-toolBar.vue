...
<template>
  <v-toolbar flat  class="toolbar-ext" height="50px">
    <v-toolbar-items>
      <div class="net-tb-search align-self-center ml-4 relative gd-tb-search">
        <input 
          v-model="searchText"
          v-on:keyup.enter="onEnter"
          type="text" 
          :style="{width: searchWidth + 'px'}"
          :class="[searchText != '' && searchText != net_filter.search ? 'blue700 blue000-b': 'grey700']"
          class="net-tb-search__input border grey300-border px-2 py-1 br-2 fw-b fs-s gd-tb-search"
          placeholder="Search"
          @focus="searchFocus()"
          @blur="searchBlur()"
          >
          <i v-show="searchText != ''" class="net-tb-search__icon icon-Close py-1 px-1 pointer" :class="[searchText != '' && searchText != net_filter.search ? 'blue700-i blue000-b': 'grey500-i white']" @click="searchText='', onEnter()"></i>
      </div>
      <v-flex srink  class="align-self-center">
        <v-btn-toggle v-model="filterToggle">        
          <v-btn class="normal-text px-2 mx-4 border-none gd-tb-filter" :class="{'blue600 blue100-b': net_filter.use, 'grey600 white': !net_filter.use}" height="24px">
            <i class="icon-filter mr-1" :class="{'blue600-i': net_filter.use}"></i>
            <span>Filters</span>
          </v-btn>
        </v-btn-toggle>
      </v-flex>
...
export default {
  ...
  computed:{
    ...mapGetters(['net_itemsFiltered', 'net_filter', 'lctn_defaultResourceProfile']),

    filterToggle: {
      get: function (){
        return this.net_filter.use ? 0 : null;
      },

      set: function(value){
        let f = this.net_filter;
        f.use= isNullOrUndefined(value) ? false : true;     
        this.net_setFilter(f);
      }
    }
  },
  methods:{
    ...mapActions(['net_deleteConfirm', 'net_create', 'net_setFilter']),
    ...
    onEnter(){
      var filter = this.net_filter;
      filter.search = this.searchText;
      this.net_setFilter(filter);
    },
    ...
    exportCsv(){
      var toCsv = this.net_itemsFiltered.map(m=>{ return { 
        NET_NAME       : m.name, 
        NET_TYPE       : m.type, 
